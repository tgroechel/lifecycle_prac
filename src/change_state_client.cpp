...
static constexpr char const *node_change_state_topic =
    "async_lc_node/change_state";
static constexpr char const *node_cancel_transition_topic =
    "async_lc_node/cancel_transition";
...
public:
  ...
  unsigned int get_state(std::chrono::seconds time_out = 3s) {
    ...
    if (!client_get_state_->wait_for_service(time_out)) {
      RCLCPP_WARN(get_logger(), "Service %s is not available.",
    ...
    if (future_result.get()) {
      ...
      RCLCPP_WARN(get_logger(), "Failed to get current state for node %s",
                  lifecycle_node);
      return lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN;
    }
  }
  ...
  bool cancel_transition(std::chrono::seconds timeout_sec = 10s) {
    auto request =
        std::make_shared<lifecycle_msgs::srv::CancelTransition::Request>();
    ...
    if (future_result.get()->success) {
      RCLCPP_INFO(get_logger(), "Transition successfully cancelled.");
      return true;
    } else {
      RCLCPP_WARN(get_logger(), "Failed to cancel transition with reason: %s",
  ...
  void change_state_w_future_request(std::uint8_t transition) {
    ...
    auto response_received_callback =
        [logger = this->get_logger()](ServiceResponseFuture future) {
          ...
          RCLCPP_INFO(logger, "Transition (%d) request was a %s",
