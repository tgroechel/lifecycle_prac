...
public:
        ...
        intra_process_comms))
  {
    register_async_on_configure(
      std::bind(
        &LifecycleTalker::on_configure_async,
        this, std::placeholders::_1,
        std::placeholders::_2));
    register_async_on_activate(
      std::bind(
        &LifecycleTalker::on_activate_async,
        this, std::placeholders::_1,
        std::placeholders::_2));
    ...
    transition_cancel_monitoring_timer_ = create_wall_timer(
      std::chrono::milliseconds{100}, [this, change_state_hdl]() {
        if (!change_state_hdl->is_executing()) {
          ...
          RCLCPP_INFO(
            this->get_logger(), "Handle cancel: pruned %ld request",
            num_pruned_req);
    ...
    auto response_received_callback =
      ...
      future) {
        if (change_state_hdl->is_executing()) {
          ...
          RCLCPP_INFO(
            logger, "Received parameter response: %s",
  ...
  on_shutdown(const rclcpp_lifecycle::State & state)
  {
    ...
    RCUTILS_LOG_INFO_NAMED(
      get_name(), "on shutdown is called from state %s.",
      state.label().c_str());
  ...
  void publish()
  {
    ...
    if (!pub_->is_activated()) {
      RCLCPP_INFO(
        get_logger(), "Lifecycle publisher is currently "
  ...
  {
    RCLCPP_INFO(
      this->get_logger(), "LC not blocked, in state (%s)",
